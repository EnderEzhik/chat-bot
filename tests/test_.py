...
@pytest.mark.parametrize("message_text, expected_answer", [
    ("ПриВЕТ", "Привет! Я бот тех. поддержки солнца. Что я могу для тебя сделать?"),
    ("ПОКА", "Пока!")
])
...
def test_get_history_messages(jwt_token, session_id):
    ...
    assert history[1].get("text") == "Привет! Я бот тех. поддержки солнца. Что я могу для тебя сделать?"
